...
private:
    ...
    bool isItemInTree(const ItemType& item, TreeNode*& t);
...
template<class ItemType>
bool BST_312 <ItemType>::isItemInTree(const ItemType& item, TreeNode*& t) {
    if (t == NULL){
        return false;
    }
    else if(item < t->data){
        return isItemInTree(item, t->left);
    }
    else if(item > t->data){
        return isItemInTree(item, t->right);
    }
    else{
        return true;
    }
